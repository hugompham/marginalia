		...
		<Card padding="lg">
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="text-sm text-secondary">Signed in via {data.user?.app_metadata?.provider ?? 'email'}</p>
				</div>
			...
			<Card padding="lg">
				<div class="flex items-start justify-between">
					...
					<div class="flex items-center gap-sm">
						...
						{:else}
							<Button size="sm" onclick={() => openAddKeyModal('openai')}>
								Add Key
							</Button>
						{/if}
					</div>
			...
			<Card padding="lg">
				<div class="flex items-start justify-between">
					<div class="flex items-start gap-md">
						<div class="p-sm rounded-button bg-subtle">
							<svg class="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
								<path d="M17.304 3.541l-5.083 16.918H8.691L3.609 3.541h3.492l3.25 11.705 3.25-11.705h3.703zM20.391 20.459l-2.031-6.163h-1.648l2.031 6.163h1.648z"/>
							</svg>
						</div>
						<div>
...
<Modal bind:open={showApiKeyModal} title="Add API Key">
	...
	>
		...
		<div class="space-y-lg">
			<p class="text-secondary">
				Add your {apiKeyProvider === 'openai' ? 'OpenAI' : 'Anthropic'} API key.
			...
			<div>
				<label for="model" class="block text-sm font-medium text-primary mb-sm">
					Model
				</label>
	...
	{#snippet footer()}
		<div class="flex justify-end gap-md">
			<Button variant="secondary" onclick={() => (showApiKeyModal = false)}>
				Cancel
			</Button>
...
<Modal bind:open={showDeleteConfirm} title="Delete Account">
	<p class="text-secondary">
		...
		All your collections, highlights, and cards will be permanently deleted.
	</p>
